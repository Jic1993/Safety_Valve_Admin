<template>
  ...
  <el-row :span="24" class="toolbar">
    <el-button type="primary" @click="issueBtn" :disabled="this.selected.length===0">批量签发</el-button>
  </el-row>
  <!-- 表格 -->
...
<script>
import { getTestList, issueTest } from '../api/api';
...
export default {
  ...
  methods: {
    ...
    issueBtn() {
      ...
      issueTest(param)
        .then(response => {
          console.log('response issueTest: ', response);
